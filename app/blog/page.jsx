...
export default async function BlogListingPage() {
  ...
  return (
    <div className="bg-gray-50 min-h-screen mt-[-32] md:mt-[-20px] ">
      {/* Hero section */}
      <div className="bg-gradient-to-r from-[#0A9DB2] to-[#0A9DB2] md:px-12 text-white py-12">
              ...
              <div key={blog.slug} className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col hover:shadow-lg transition-shadow">
                <div className="p-6 flex-grow">
                  <Link href={`/blog/${blog.slug}`}>
                    <h2 className="text-xl font-bold text-gray-800 mb-3 hover:text-[#0A9DB2]">
                      {blog.title}
                    </h2>
                  </Link>
                ...
                <div className="px-6 py-4 bg-gray-50 border-t border-gray-100">
                  <Link 
                    href={`/blog/${blog.slug}`}
                    className="text-[#0A9DB2] hover:text-[#1c6570] font-medium flex items-center text-sm"
                  >
                    Read Article
                    <svg 
